        ...
        try:
            ...
            for chat_id in list(user_status.keys()):
                try:
                    ...
                    # انتظار نصف ثانية لتفادي ضغط السيرفر
                    time.sleep(0.5) 
                except Exception:
                    # إذا حظر الطالب البوت، نتجاهله لضمان استمرار السيرفر
                    continue 
                    
        except Exception as e:
            # في حال حدث خطأ تقني عام، ننتظر 10 ثوانٍ
            print(f"حدث خطأ في نظام التذكير: {e}")
            time.sleep(10)

# --- تشغيل نظام التذكير في الخلفية وبدء البوت ---
import threading
try:
...
print("🚀 البوت الآن يستقبل الرسائل...")
